...
def getdatapemain_multithread(n_thread=1, is_secure=False):
    texec = dict()
    hasil = [None] * n_thread
    ...
    for k in range(n_thread):
        waktu = time.time()
        #bagian ini merupakan bagian yang mengistruksikan eksekusi fungsi getdatapemain secara multithread
        texec[k] = threading.Thread(target=getdatapemain, args=(randint(1,8), is_secure, hasil, k))
        texec[k].start()
    ...
    print("Response untuk request dengan jumlah " + str(n_thread) + " thread")
    for k in range(n_thread):
